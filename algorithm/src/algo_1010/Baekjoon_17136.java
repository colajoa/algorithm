...
public class Baekjoon_17136 {
	...
	static void check(int one, int two, int three, int four, int five, int count) {
		if(checkFull()) {
			...
			for(int c=0; c<N; c++) {
				if(five > 0) {
					if(checkSquare(r,c,5)) {
						cover(r,c,5);
						check(one, two, three, four, five-1, count+1);
						recover(r,c,5);
					}
				}
				...
				if(four > 0) {
					if(checkSquare(r,c,4)) {
						cover(r,c,4);
						check(one, two, three, four-1, five, count+1);
						recover(r,c,4);
					}
				}
				...
				if(three > 0) {
					if(checkSquare(r,c,3)) {
						cover(r,c,3);
						check(one, two, three-1, four, five, count+1);
						recover(r,c,3);
					}
				}
				...
				if(two > 0) {
					if(checkSquare(r,c,2)) {
						cover(r,c,2);
						check(one, two-1, three, four, five, count+1);
						recover(r,c,2);
					}
				}
				...
				if(one > 0) {
					if(checkSquare(r,c,1)) {
						cover(r,c,1);
						check(one-1, two, three, four, five, count+1);
						recover(r,c,1);
					}
				}
			}
